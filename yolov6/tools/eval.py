...
def get_args_parser(yaml_file, vid, camera, add_help=True):
    ...
    parser.add_argument('--weights', type=str, default= 'D:\\!cechy\\YOLOv6\\runs\\train\\exp3\\weights\\best_ckpt.pt', help='model.pt path(s)')
    parser.add_argument('--batch-size', type=int, default=32, help='batch size')
    parser.add_argument('--img-size', type=int, default=160, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.1, help='confidence threshold')#3
    parser.add_argument('--iou-thres', type=float, default=0.2, help='NMS IoU threshold')#4
    ...
    parser.add_argument('--save_dir', type=str, default='F:/runs/val/_21_04_2024', help='evaluation save dir')
    parser.add_argument('--name', type=str, default= vid + '-' + camera, help='save evaluation results to save_dir/name')
    parser.add_argument('--test_load_size', type=int, default=160, help='load img resize when test')
...
if __name__ == "__main__":
    ...
    - test_load_size
    """
    # point YAMLs files to process with YOLO (single YAML - single camera & single word)
    all_yaml = glob.glob('D:\\!cechy\\!data_05022024\\yolo_yaml_files_part7\\*.yaml')
    ...
    for yamls in all_yaml:
        ...
        # path dir to images for specific word and single camera
        pathImgDir = 'D:\\!cechy\\!data_05022024\\yolo_preprocessed_frames\\' + kidID + '\\' + kidID + '-' + kidRecID + \
                     '\\' + kidWord + '\\' + kidCamera[:-5] + '\\images\\'

        folder_with_imgs = glob.glob(pathImgDir + '*.png')
        isExistprepr = os.path.exists(pathImgDir.replace('images', 'labels'))
        if isExistprepr is False:
            os.makedirs(pathImgDir.replace('images', 'labels'))

        # prepare empty labels - YOLO requires it
        for imgs in folder_with_imgs:
            with open(imgs.replace('images', 'labels').replace('png', 'txt'), 'w') as fp:
                pass

        args = get_args_parser(yamls, kidID + '-' + kidRecID + '-' + kidWord, kidCamera[:-5])
        main(args)