...
yaml_files = glob.glob('D:\\!cechy\\!data_PB\\yolo_yaml_files_pierwszaczesc\\5758\\00058*.yaml')
for yamls in yaml_files:
    ...
    try:
        ...
        # yolo_preprocessed_frames_newone
        pathImgDir = 'D:\\!cechy\\!data_PB\\yolo_preprocessed_frames\\' + kidID + '\\' + kidID + '-' + kidRecID + \
                     '\\' + kidWord + '\\' + kidCamera[:-5]

        print('Vid ' + yamls.split('\\')[-1][:-5] + ' processing...')
        df = pd.read_json(
            'D:\\!cechy\\YOLOv6\\tools\\runs\\val\\!58\\' + yamls.split('\\')[-1][:-5] + '\\predictions.json')
        ...
        for idx in range(0, len(df)):
            ...
            if df.category_id[idx] == 0:
                ...
                if len(frameIDs[0]) > 1:
                    for idxFrame in frameIDs[0]:
                        if df.category_id[idxFrame] == 0:
                            if counterFramesIDs == 0:
                                """df.drop(labels=idxFrame)
                                df.to_json(
                                        'D:\\!cechy\\YOLOv6\\tools\\runs\\val\\temp_new182\\' + yamls.split('\\')[-1][
                                                                                            :-5] + '\\predictions.json',
                                        orient='records')"""
                                if df.score[idxFrame] < 0.75:
                                    ...
                                    flag_noBB = True
                                    df.to_json(
                                        'D:\\!cechy\\YOLOv6\\tools\\runs\\val\\!58\\' + yamls.split('\\')[-1][
                                                                                        :-5] + '\\predictions.json',
                                        orient='records')
                                else:
                    ...
                    if df.score[idxFrame] < 0.75:
                        df.loc[idxFrame, 'category_id'] = 5
                        df.to_json('D:\\!cechy\\YOLOv6\\tools\\runs\\val\\!58\\' + yamls.split('\\')[-1][
                                                                                   :-5] + '\\predictions.json',
                                   orient='records')
                        flag_noBB = True
                    else:
